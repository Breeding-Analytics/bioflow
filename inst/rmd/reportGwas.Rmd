...
if (is.null(result)){
  ...
  traits <- unique(result$modeling[which(result$modeling$analysisId == idGwas  & result$modeling$environment == "across"),"trait"]) # unique(pred$trait)
...
if (is.null(result)){
  ...
  if(length(field) > 1){mod <- "sta"}else{mod <- "mta"}
  pred.gwas <- result$predictions
  pred <- pred.gwas[pred.gwas$module == mod,]
...
if(length(fieldManP) > 1){mod <- "sta"}else{mod <- "mta"}
...
Man <- list()
  for(iTrait in traitsManP){
    for(iField in fieldManP){
      ...
      pred.gwas.i$o <- o
      pred.gwas.i$e <- e
      QQ[[iField]] <- ggplot2::ggplot(pred.gwas.i, aes(x=e, y=o)) + ggplot2::geom_point() +
        ggplot2::geom_abline(intercept = 0, slope = 1, col = "deepskyblue2") + ggplot2::theme_classic() +
        ggplot2::expand_limits(x=c(0,round(max(o)+1)),y=c(0,round(max(o)+1))) + ggplot2::scale_x_continuous(breaks = seq(0,round(max(o)+1))) + ggplot2::scale_y_continuous(breaks = seq(0,round(max(o)+1))) + ggplot2::theme(axis.text = element_text(colour = "black"), axis.title = element_text(colour = "black")) + 
        ggpubr::border() +
    ...
    Markers <- result$data$geno
    marker_map <- result$metadata$geno
    marker_map <- marker_map[,c(1:3)]
    modificationsMarkers <- result$modifications$geno
    ...
    id <- result$status[which(result$status$module == mod),"analysisId"]
    id <- id[length(id)]
    ...
    Man[[iField]] <- manhattanPlot(don, threshold = -log10(p_threshold), field = iField)
  }
  }
...
# }
    ...
    plotly::renderPlotly({
      ...
      pred.gwas.i$o <- o
      pred.gwas.i$e <- e
      qq <- ggplot2::ggplot(pred.gwas.i, aes(x=e, y=o)) + ggplot2::geom_point() +
        ggplot2::geom_abline(intercept = 0, slope = 1, col = "deepskyblue2") + ggplot2::theme_classic() +
        ...
        # ggplot2::labs(x = expression(Expected ~ ~-log[10](italic(p))), y = expression(Observed ~ ~-log[10](italic(p)))) +
        ggplot2::theme(axis.text = element_text(colour = "black"), axis.title = element_text(colour = "black")) + 
        ggpubr::border() #+
        #ggplot2::labs(x = expression(Expected ~ ~-log[10](italic(p))), y = expression(Observed ~ ~-log[10](italic(p))))
      ...
      Man[[inputField]] <- manhattanPlot(don, threshold = -log10(p_threshold)) # , field = inputField 
      #}
...
s <- ggplot2::ggplot(data = top5matrix, aes(x = factor(value), y = top5matrix[,2])) + # [[inputField]]
  #labs(x = "Genotype", y = "Phenotype") + 
  theme_bw() + 
  geom_boxplot(fill='#A4A4A4') + geom_jitter(alpha = .1) +
  theme(axis.text = element_text(color="black"),
        axis.title = element_text(color="black", size = 10)) + 
  ggpubr::border() + ggplot2::labs(x = "Genotype", y = "Phenotype") + facet_grid(. ~ topmarkers, scales='free')
...
if (is.null(result)){
  ...
  pred.gwas <- subset(pred.gwas, select = -c(module,analysisId,pipeline,gid,mother,father,entryType))
