...
if (is.null(result)){
  ...
  if(length(field) > 1){mod <- "sta"}else{mod <- "mtaLmms"}
  pred.gwas <- result$predictions
...
if(length(fieldManP) > 1){mod <- "sta"}else{mod <- "mtaLmms"}
...
Man <- list()
  for(iTrait in traitsManP){
    ...
    Markers <- result$data$geno_imp[[ as.character(as.integer(idMarkers)) ]]
