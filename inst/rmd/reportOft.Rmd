...
### remove the outliers
dt_clean <- dt_orig
rowPheno <- result$modifications$pheno[which(result$modifications$pheno$analysisId == params$version),"row"]
traitPheno <- result$modifications$pheno[which(result$modifications$pheno$analysisId == params$version),"trait"]
...
if (is.null(result)){
  ...
  ## Mean
  pred.traits.mean <- aggregate(predictedValue ~ environment + trait, data = pred.traits, function(x) mean(x,na.rm=T))
  ...
  pred.traits.summary <- merge(x = pred.traits.min, y = pred.traits.mean, by = c("environment","trait"), all.x = TRUE)
  pred.traits.summary <- merge(x = pred.traits.summary, y = pred.traits.max, by = c("environment","trait"), all.x = TRUE)
  pred.traits.summary <- merge(x = pred.traits.summary, y = metrics.h2, by = c("environment","trait"), all.x = TRUE)
  pred.traits.summary <- merge(x = pred.traits.summary, y = metrics.CV, by = c("environment","trait"), all.x = TRUE)
  pred.traits.summary <- merge(x = pred.traits.summary, y = metrics.r2, by = c("environment","trait"), all.x = TRUE)
  pred.traits.summary <- merge(x = pred.traits.summary, y = metrics.Vg, by = c("environment","trait"), all.x = TRUE)
  pred.traits.summary <- merge(x = pred.traits.summary, y = metrics.Vr, by = c("environment","trait"), all.x = TRUE)
