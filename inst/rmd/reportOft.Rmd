...
qaID <- result$modeling[which(result$modeling$module == "sta" & result$modeling$analysisId == params$version & result$modeling$parameter == "analysisId"), "value"]
...
if(!is.null(params$mdisease)){
  colnames(dt_orig)[which(names(dt_orig) == params$mdisease)] <- "MDiseases"
  colnames(dt_orig)[which(names(dt_orig) == params$tdisease)] <- "Type_Disease"
  colnames(dt_orig)[which(names(dt_orig) == params$sdisease)] <- "Disease_Severity"
}
...
dt_sta_pred <- dt_sta_pred[which(dt_sta_pred$analysisId == params$version),]

dt_sta_met <- result$metrics[which(result$metrics$module == "sta"),]
dt_sta_met <- dt_sta_met[which(dt_sta_met$environment %in% fieldinst),]
dt_sta_met <- dt_sta_met[which(dt_sta_met$analysisId == params$version),]
...
if (is.null(result)){
  ...
  metrics <- dt_sta_met
  metrics <- metrics[metrics$analysisId == params$version,]
