...
```{r getData, error = FALSE}
### loading the RData
obj <- result
dt_orig <- obj$data$pheno

### change column names for mapping
paramsPheno <- obj$metadata$pheno
...
### remove the outliers
dt_clean <- dt_orig
rowPheno <- obj$modifications$pheno$row
traitPheno <- obj$modifications$pheno$trait
...
dt_sta_pred <- obj$predictions[which(obj$predictions$module == "sta"),]
dt_sta_pred <- dt_sta_pred[which(dt_sta_pred$environment %in% fieldinst),]

dt_sta_met <- obj$metrics[which(obj$metrics$module == "sta"),]
...
varInfo <- dt_clean %>% count(environment,entryType,designation,year) %>% 
  mutate(color = case_when(entryType == "TEST VARIETY" ~ "#2835e6",
                           entryType == "LOCAL VARIETY" ~ "#038542",
                           entryType == "BENCHMARK VARIETY" ~ "#e65628")) %>% 
  mutate(year = as.numeric(year)) %>% arrange(environment, designation, entryType)  
  ...
  for (j in seq_along(environment)) { 
    ...
    if(nrow(UPR %>% filter(Control=="MFU")) > 1){
      ...
      for (k in nrow(UPR %>% filter(Control=="MFU"))){
        assign(paste0("sigU_TVFV",k),subset(CI_TV,as.logical(MFU_vals[k] >= CI_TV$LL)==F))
...
for (j in seq_along(RiceEnvs)) {
 ...
 bar_dse[[j]] <-  plot_ly(Var_Dse, x = ~reorder(designation, -predictedValue), y = ~Yes,
                  ...
                  layout(xaxis = list(title = "designation"), 
                         yaxis = list(title = "Predicted Value")) %>% layout(legend = list(orientation = 'h', xanchor = "center", x = 0.5, y= 1)) %>% layout(title = "Major Disease Occurrence", margin = m)
...
for (i in 1:length(traits)) {
  ...
  for (j in 1:(nlevels(as.factor(dt_clean$environment)))) {
    ...
    BLUPsBG[[i]][[j]] <- plot_ly(BLUPs_BG, x = ~reorder(designation, -predictedValue),
                                 marker = list(color = '#038542', width=3), width = 800, 
                                 height = 800) %>% 
                         add_trace(y = ~predictedValue,type = "bar", name = "Predicted Value",
                                   text = round(BLUPs_BG$predictedValue,2), 
                                   textposition = 'outside',
                                   textfont = list(color = 'darkblue', size = 12)) %>%  
                         add_trace(y = ~gainOverLoc, mode = "lines+markers+text", type = "scatter",
                                   yaxis = "y2", name = "% gain over FV", 
                         ...
                         layout(legend = list(orientation = 'h', xanchor = "center", x = 0.5, y= 1)) %>% layout(title = "Predicted performance", margin = m)
