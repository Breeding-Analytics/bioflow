...
if (is.null(result)){
  ...
  for(i in 1:length(paramsPheno$parameter)){
    if((paramsPheno$parameter[i] %in% colnames(mydata)) & 
       (paramsPheno$parameter[i] != paramsPheno$value[i])){
      colnames(mydata) <- replace(colnames(mydata), 
                                 colnames(mydata) == paramsPheno$parameter[i],
                                 paste0(paramsPheno$parameter[i],"_dup"))
    }
  }
