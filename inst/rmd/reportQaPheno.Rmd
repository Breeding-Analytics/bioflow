...
if (is.null(result)){
  ...
  idQa <- result$status[which(result$status$module %in% c("qaRaw","qaFilter","qaDesign")),"analysisId"];
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iTrait in traitsBoxplot){
      ...
      moTrait <- mo[which(mo$trait == iTrait),"row"] # mo by trait
      if(nrow(mo) > 0){mydata$color[which(mydata$rowindex %in% unique(moTrait))]="tagged"}
      mydata$color <- as.factor(mydata$color)
      ...
      p <- ggplot(mydata, aes(x=as.factor(environment), y=predictedValue)) +
        ...
        theme_classic()+
        geom_jitter(aes(colour = color), alpha = 0.5) +
    ...
    shiny::renderPlot({
      ...
      moTrait <- mo[which(mo$trait == input$traitQaBox),"row"] # mo by trait
      if(nrow(mo) > 0){mydata$color[which(mydata$rowindex %in% unique(moTrait))]="tagged"}
      mydata$color <- as.factor(mydata$color)
