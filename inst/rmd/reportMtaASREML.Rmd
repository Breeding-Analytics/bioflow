...
if (is.null(result)){
  ...
  term<-grep("predictedValue", names(pred.mta.wide), ignore.case = TRUE)
  traitsT<-gsub("predictedValue.", "", names(pred.mta.wide))
  traitsT<-traitsT[term]  
  names(pred.mta.wide) <- c("designation","environment","effectType","entryType",traitsT)
  ...
  pred.mta.wide2_trait <- pred.mta.wide2 %>% select(c("designation", all_of(traitsT))) %>%
    group_by(designation) %>% 
    summarise(across(traitsT, coalesce_by_column)) 
  pred.mta.wide2_meta <- pred.mta.wide2 %>% select(!all_of(traitsT)) %>%
