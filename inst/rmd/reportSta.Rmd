...
if (is.null(result)){
  ...
  # metrics <- metrics[metrics$analysisId == idSta,]
  # metrics.h2 <- metrics[metrics$parameter%in% c("plotH2", paste("plotH2",c("designation","mother","father"), sep="_") ),]
  # metrics.h2 <- subset(metrics.h2, select = c(trait,environment,value))
  # colnames(metrics.h2)[3] <- "H2"
  # ## CV
  # metrics.CV <- metrics[metrics$parameter %in% c("CV", paste("CV",c("designation","mother","father"), sep="_") ),]
  # metrics.CV <- subset(metrics.CV, select = c(trait,environment,value))
  # colnames(metrics.CV)[3] <- "CV"
  # ## r2
  # metrics.r2 <- metrics[metrics$parameter %in% c("r2", paste("r2",c("designation","mother","father"), sep="_") ) ,]
  # metrics.r2 <- subset(metrics.r2, select = c(trait,environment,value))
  # colnames(metrics.r2)[3] <- "r2"
  # ## Vg
  # metrics.Vg <- metrics[metrics$parameter%in% c("V_designation", paste("V_designation",c("designation","mother","father"), sep="_") ),]
  # metrics.Vg <- subset(metrics.Vg, select = c(trait,environment,value))
  # colnames(metrics.Vg)[3] <- "V_genetic"
  # ## Vr
  # metrics.Vr <- metrics[metrics$parameter%in% c("V_residual", paste("V_residual",c("designation","mother","father"), sep="_") ),]
  # metrics.Vr <- subset(metrics.Vr, select = c(trait,environment,value))
  # colnames(metrics.Vr)[3] <- "V_residual"
  # 
  ...
  # numeric.output <- c("min","mean","max","H2","CV","r2","V_designation","V_residual")
...
if (is.null(result)){
  ...
  df2 <- df2[df2$parameter %in% c("Vr","Vg","V_designation","V_residual","V_repF","V_iBlockF","V_rowF","V_colF", "V_s(row, col)",
                                  apply(expand.grid( c("Vr","Vg","V_designation","V_residual","V_repF","V_iBlockF","V_rowF","V_colF", "V_s(row, col)"), c("designation","mother","father")),1,function(f){paste(f,collapse = "_")})
  ), ]
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    plotly::renderPlotly({
      ...
      p_proportion_variance <- ggplot2::ggplot(data=df3, ggplot2::aes(x=environment, y=valueProp, fill=parameter)) +
        ggplot2::geom_bar(stat="identity") +
        # ggplot2::scale_fill_brewer(palette="Accent") +
        ggplot2::theme(axis.text.x = ggplot2::element_blank()) +
        ggplot2::xlab("") + ggplot2::ylab("Proportion of variance") 
      p_value_variance <- ggplot2::ggplot(data=df3, ggplot2::aes(x=environment, y=value, fill=parameter)) +
        ggplot2::geom_bar(stat="identity") +
        # ggplot2::scale_fill_brewer(palette="Accent") +
        ggplot2::theme(axis.text.x = ggplot2::element_blank()) +
        ggplot2::guides(fill = "none") +
  ...
  if("params" %in% ls()){  # if user will download the report
    ...
    for(iParam in unique(parameters)){
      ...
      for(iView in c("environment","trait") ){
        cat("\n\n####", paste0("by-",iView,"-" ,iParam), "{.tabset .tabset-pills}       \n\n")
...
if (is.null(result)){
  ...
  idSta <- idSta[length(idSta)]
  modeling <- modeling[which(modeling$analysisId == idSta),"trait"]
  traitsComp <- unique(setdiff(modeling, "inputObject"))
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    plotly::renderPlotly({
      ...
      p1 <- ggplot2::ggplot(temp.predComp.box, ggplot2::aes(x=reorder(entryType, -predictedValue), y=predictedValue) ) +
        ggplot2::geom_boxplot(alpha = 0.5, show.legend = TRUE, ggplot2::aes(colour=factor(entryTypeSimple))) +  ggplot2::facet_grid(~environment) +
        ggplot2::theme(axis.text.x = ggplot2::element_blank(), legend.position = "bottom", legend.title = element_blank() ) + 
        ggplot2::labs(x="", y="Trait value") +  
