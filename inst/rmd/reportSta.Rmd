...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for (j in 1:length(staTraits2)){
      ...
      print(htmltools::tagList( plotly::subplot(pplotly::ggplotly(p_proportion_variance), plotly::ggplotly(p_value_variance), nrows=2, shareX = TRUE, shareY = TRUE) ) )
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iTrait in traitsComp){
      ...
      p2 <- ggplot2::ggplot(
        ...
        ggplot2::theme(axis.text.x = element_text(angle = 45, vjust = 0.5, size = 7, face = "bold"), legend.position = "bottom", legend.title = element_blank() ) + ggplot2::facet_grid(~environment, scales = "free_x") +
