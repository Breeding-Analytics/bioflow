...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    if(length(which( result$metadata$pheno$parameter %in% c("row","col") ))==2 ){ # user mapped the columns
      ...
      # coordNames <- result$metadata$pheno[result$metadata$pheno$parameter %in% c("row","col"),"value" ]
      idQa <- result$modeling[which(result$modeling$analysisId == idSta &
                                      result$modeling$parameter == "analysisId"),"value"]
      
      cleaning <- result$modifications$pheno[which(result$modifications$pheno$analysisId %in% idQa),]
      
      for (iID in idQa){
        module <- result$status[which(result$status$analysisId == iID),"module"]
        ...
        if(module == "qaDesign"){
          if(nrow(cleaning[which(cleaning$module == "qaDesign"),] > 0)){
            ...
            for (iDes in unique(cleaningSubDes$trait)){
              outDes <- which(pheno$rowindex %in% cleaningSubDes[which(cleaningSubDes$trait == iDes),
                                                                 "row"])
              pheno[outDes,iDes] <- NA
            }
          }
        } else{
          if(nrow(cleaning[which(cleaning$module != "qaDesign"),] > 0)){
            cleaningSub <- cleaning[which(cleaning$trait %in% traitFieldView),]
            
            for (iTrait in traitFieldView){
              out <- which(pheno$rowindex %in% cleaningSub[which(cleaningSub$trait == iTrait),
                                                           "row"])
              pheno[out,iTrait] <- NA
            }
          }
      ...
      if( any( apply(pheno[,c("row", "col")],2, enhancer::propMissing) == 1) ){ # we can't proceed, there's no information in one of the coordinates
        shinyjs::hide()
