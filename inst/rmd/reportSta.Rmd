...
printTable <- function(DT, rows = c('same', 'different'), pageLength = 7,
                       numericColNames = NULL, numRound = 3, ...) {

  rows = match.arg(rows)

  # For data.frames with rows for the same type of data (table of predictions)

  if (rows == 'same')
    DT <- cgiarBase::round.df(DT, numRound)
  ...
  # For data.frames with rows for different type of data (summary statistics)

  if (rows == 'different')
    table <- cgiarBase::round.dt(table, numRound)
...
if (is.null(result)){
  ...
  ## Vg
  metrics.Vg <- metrics[metrics$parameter=="V_designation",]
  metrics.Vg <- subset(metrics.Vg, select = c(trait,environment,value))
  colnames(metrics.Vg)[3] <- "V_designation"
  ## Vr
  metrics.Vr <- metrics[metrics$parameter=="V_residual",]
  ...
  if("params" %in% ls()){ # we will download the document
    printTable(unique(pred.traits.summary), 'different')
  }else{
    DT::renderDT(cgiarBase::round.dt(DT::datatable(
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    DT::renderDT(DT::datatable(
      cgiarBase::round.df(predictions.designation[predictions.designation$environment==input$envSta,], 3),
      rownames = FALSE,
      class = 'cell-border stripe',
      extensions = 'Buttons',
      options = list(dom = 'Blfrtip',scrollX = TRUE,buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
