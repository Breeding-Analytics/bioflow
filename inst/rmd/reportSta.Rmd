...
if (is.null(result)){
  ...
  staTraits <- unique(modeling[which(modeling$analysisId == idSta),"trait"])
...
if("params" %in% ls()){ # we will download the document
  ...
  for (j in 1:length(traits)){
    ...
    print(htmltools::tagList(ggplotly(
      
      ggplot2::ggplot(predTrait[which(predTrait$trait %in% traits[j]),], ggplot2::aes(x=environment, y=predictedValue, label=designation) ) + 
