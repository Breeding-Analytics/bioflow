  ...
  if( length( unique(result$predictions[result$predictions$analysisId == idMta, "environment"]) ) > 2 ){
    ...
    if("params" %in% ls()){ # we will download the document
      ...
      for(iTrait in traitsBiplotAll){
        ...
        pred.mta.biplot <- pred.mta.biplot[which(apply(pred.mta.biplot,1,sommer::propMissing) == 0),,drop=FALSE]
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iTrait in traits){
      ...
      for(iEnv in envs2){
        ...
        temp.pred.box <- temp.pred.box[temp.pred.box$environment== iEnv,]
        temp.pred.box$entryTypeSimple <- LETTERS[as.numeric(as.factor(temp.pred.box$entryType))]
        ...
        temp.pred.box2$entryTypeSimple <- LETTERS[as.numeric(as.factor(temp.pred.box2$entryType))]
        ...
        print(htmltools::tagList( plotly::subplot(plotly::ggplotly(p2), plotly::ggplotly(p1), shareX = TRUE, nrows=1, widths = c(0.75,0.25) ) ) )
    ...
    plotly::renderPlotly({
      ...
      temp.pred.box <- pred[pred$trait== input$traitMta2 & pred$environment == input$envCovMta2,]
      temp.pred.box$entryTypeSimple <- LETTERS[as.numeric(as.factor(temp.pred.box$entryType))]
