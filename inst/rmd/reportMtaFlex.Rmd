...
if (is.null(result)){
  ...
  if( any( unlist(lapply( as.list( unique(result$metadata$weather[result$metadata$weather$parameter == "trait", "value" ]) ), function(x){
    sum(grepl(x,traitsInPreds))
  })) > 0 )
  ){
    result$status <- result$status#[1:6,]
    idMta <- result$status[which(result$status$module == "mtaFlex"),"analysisId"]
    ...
    if("params" %in% ls()){ # we will download the document
      ...
      if(length(unique(pred$designation)) > 1000){ # go fixed format
        ...
        shiny::renderPlot({ # regular ggplot
          ...
          stabilityTrait <- ifelse(isRRBLUP, paste0(input$traitMta3,"_QTL", input$envCovMta3), paste0(input$traitMta3,"-", input$envCovMta3) )
          if(stabilityTrait %in% traitsInPreds){ # if the user modeled that existing covariate in the GxE
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iEnvTrait in envs3){
      ...
      if(nrow(predictions.gcorrT)){
        ...
        corr <- round(stats::cor(wideCorTrait2, use="pairwise.complete.obs"),2)
        
        mydata4CorTraits <- cgiarBase::matToTab(corr)
