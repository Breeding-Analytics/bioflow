...
if (is.null(result)){
  ...
  modifications <- result$modifications$geno_filtering
  print(modifications)
  ...
  traitsTable <- unique(modifications[which( modifications$analysisId == idQa),"metric"])
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    DT::renderDT(printTable(modifications[modifications$metric==input$traitQa,],
                        autoWidthOpt = FALSE, scrollXOpt = TRUE,
                        colNames = colnames(modifications[modifications$metric==input$traitQa,]),
