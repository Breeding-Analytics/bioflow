...
if(is.null(result)){
  ...
  gl <- result$data$geno
  idMarkers <- sort(result$status[which(result$status$module == "qaGeno"),"analysisId"])
  ...
  fgl <- cgiarGenomics::recalc_metrics(gl[t_ind, t_loc])
  
  imp_gl <- cgiarGenomics::recalc_metrics(result$data$geno_imp[[as.character(round(idMarkers))]])
