...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    DT::renderDT({DT::formatRound(DT::datatable(
      predictions[predictions$trait==input$traitQa,c("trait","gid","designation","mother","father","predictedValue")],
      rownames = FALSE,
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    plotly::renderPlotly({
      ...
      predictions2 <- predictions[which(predictions$trait == input$traitQa),]
      p <- ggplot(predictions2, aes(x=designation, y=predictedValue  ) ) +
        # geom_smooth(method = "lm", formula = y ~ poly(x, 2)) + 
        theme_classic()+
         geom_point(alpha = 0.6)+
