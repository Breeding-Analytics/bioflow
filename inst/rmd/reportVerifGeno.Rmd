...
The following plot shows how many F1 individuals derived from each parent received a "PASS" status or failed. Parents that resulted in mostly failed F1s are more likely to have genotyping errors
...
The following table shows modifications done to the genotypic data during the F1 (QA/QC analysis)
...
if (is.null(result)){
  ...
  # numericalColumns2 <- c("value") 
  modVeri <- result$modifications$geno[which(result$modifications$geno$analysisId == idQa),]
