...
if (is.null(result)){
  ...
  if("sample_id" %in% colnames(ped)){
    ...
    predictions_wide = merge(predictions_wide,F1_ind)
    OutputCols <- c("sample_id","designation","mother","father","status","probMatch","heteroMexp","heteroMp","heteroDeviation","avgScore","nScore2","nMarkers","HasGeno","HasPed","parHetFilter","ParentHasGeno")
    predictions_wide = predictions_wide[,OutputCols]
  }
  ...
  DT::formatStyle(
    ...
    DT::renderDT({
  if("sample_id" %in% OutputCols){
    cols <- c(OutputCols[1:5], input$traitQa)
  }else{
...
if (is.null(result)){
  ...
  # numericalColumns2 <- c("value") 
  modVeri <- result$result$modifications$geno[which(result$modifications$geno == idQa),]
