...
Understanding these data features should allow the scientist to identify which F1 individuals should be considered true F1s (PASS), which ones should be investigated further (LIKELY F1, UNCERTAIN) and which ones are not TRUE F1s (MISMATCH). This dashboard also helps users to identify problems in the input data with flags to identify PARENT FAIL due to lack of pedigree information (HasPed), due to one of the parents not passing heterozygosity threshold (parHetFilter) or due to one of the parents having no genotypic information (ParentHasGeno) and also identify ungenotyped F1 indviduals (HasGeno / NO GENO DATA).
...
if (is.null(result)){
  ...
  idx2 <- predictionsX$trait == "parHetFilter" | 
    predictionsX$trait == "ParentHasGeno" | 
    predictionsX$trait == "HasPed"
  idx3 <- predictionsX$trait == "HasGeno"
  
  
  status <- ifelse(predictionsX$predictedValue[idx3] == 0, "NO GENO DATA",
                   ifelse(predictionsX$predictedValue[idx2] == 0, "PARENT FAIL",
                   ifelse(predictionsX$predictedValue[idx] >= upperThres, "PASS",
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for (j in seq_along(traitsTable)) {
      cat("\n\n####", traitsTable[j], "{.tabset .tabset-pills}       \n\n")
      
      predictions2 <- predictionsX[predictionsX$trait == traitsTable[j], , drop = FALSE]
      # preserve original order through the join
      predictions2$..ord <- seq_len(nrow(predictions2))
      # left join by "designation"
      ...
      print(htmltools::tagList(plotly::ggplotly(p, tooltip = "text")))
      }
...
if (is.null(result)){
  ...
  OutputCols <- c("designation","mother","father","status","probMatch","heteroMexp","heteroMp","heteroDeviation","avgScore","nScore2","nMarkers","HasGeno","HasPed","ParentHetFilter","ParentHasGeno")
  ...
  DT::formatStyle(
    ...
    DT::renderDT({
  # ensure 'status' is included so we can style by it
