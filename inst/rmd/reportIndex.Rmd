...
if (is.null(result)){
  ...
  # mydataOcs <- dtIdxD[which(dtIdxD$analysisId == idOcs),setdiff(colnames(dtIdxD),c("module","analysisId"))]
  # idsPop <- unique(mydata[mydata$trait == iTrait, "designation"])
  # idsSel <- unique(c( mydataOcs[, "mother"], mydataOcs[, "father"] ) )
  # mydata[ which( mydata$designation %in% idsSel ), "selected"] <- "yes"
