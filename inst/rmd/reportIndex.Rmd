...
`r if( length(na.omit(unique(result$predictions$mother)))> 0 & length(na.omit(unique(result$predictions$father)))>0 ){"### Performance of crosses\n
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    if( length(na.omit(unique(result$predictions$mother)))> 0 & length(na.omit(unique(result$predictions$father)))>0 ){
...
if( length(na.omit(unique(result$predictions$mother)))> 0 & length(na.omit(unique(result$predictions$father)))>0 ){
  ...
  for (iTrait in traitsForBoxplot){ # 
    ...
    if(length(unique(dd$cross)) < 300){ # if there's more than 300 crosses don't plot
