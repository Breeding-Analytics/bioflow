...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    shinyjs::hide()
  }else{ # input <- list(traitPredictionsCorrelation=traits[1],boxplotMtaPredsTypeCorr=entryTypes[1])
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iType3 in entryTypes){
      ...
      for(iTrait3 in traits){
        ...
        if( all(c( ncol(pred.mta.biplot) >1, is.matrix(pred.mta.biplot) >1 ) ) ){
