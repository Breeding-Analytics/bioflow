...
if (is.null(result)){
  ...
  traits2 <- gsub("[^[:alnum:]]", "", unique(result$metadata$pheno[which(result$metadata$pheno$parameter == "trait"), "value"]))
    gxeList <- list("gxeCS" = c("environment_designation", "designation_environment", 
                  ...
                  "gxeFW" = c(paste0("value",traits2,"envIndex_designation"),paste0("designation_value",traits2,"envIndex"),
                              paste0("value",traits2,"envIndex:designation"),paste0("designation:value",traits2,"envIndex")))
...
if (is.null(result)){
  ...
  if(!is.null(result$data$weather)){ # if weather data was extracted
    ...
    if(length(c(latitude,longitude)) == 2){
      colnames(xx) <- cgiarBase::replaceValues(colnames(xx), Search = paste0("value",c(latitude,longitude),"mean") , Replace = c("latitude","longitude"))
      if("params" %in% ls()){ # we will download the document
  ...
  if(gxeModelNum == 2){
    if("params" %in% ls()){ # we will download the document
      ...
      for(iTrait2 in traits){
        ...
        for(i in 1:length(level2)){
          # level2[[i]][[1]]$designation <- gsub(paste0("env",gsub("[^[:alnum:]]","",colnames(level2[[i]][[1]])),":"),"",
          #                                 gsub(paste0(":","env",gsub("[^[:alnum:]]","",colnames(level2[[i]][[1]]))),"",
      ...
      for(iTrait3 in traits){
        ...
        if(length(level3)>0){
          ...
          for(i in 1:ncol(pred.mta.biplot)){
            # rownames(pred.mta.biplot) <- gsub(paste0("env",gsub("[^[:alnum:]]","",colnames(pred.mta.biplot)[i]),":"),"",
            #                                   gsub(paste0(":","env",gsub("[^[:alnum:]]","",colnames(pred.mta.biplot)[i])),"",
