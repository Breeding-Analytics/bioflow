...
if (is.null(result)){
  ...
  predSplit <- split(pred, pred[,"entryType"])
  ...
  predSplitSplitWide <- lapply(predSplitSplit, function(y){lapply(y,function(x){
    ...
    ninds <- tableForm[1]; nEnvs <- length(tableForm)
    idsmock <- rep( paste0("id",1:ninds), nEnvs)
    ...
    wide0 <- wide0[,-1,drop=FALSE]; rownames(wide0) <- x$designation[1:ninds]
    colnames(wide0) <- gsub("predictedValue_","",colnames(wide0))
    return(wide0)
  }
  )})
  predSplitSplitWide2 <- lapply(predSplitSplit2, function(y){
    lapply(y,function(x){
      tableForm <- table(x$trait)
      ...
      wide0 <- wide0[,-1,drop=FALSE]; rownames(wide0) <- unique(x$designation)
      colnames(wide0) <- gsub("predictedValue_","",colnames(wide0))
      return(wide0)
    })
  })
  entryTypes <- rev(names(predSplitSplit))
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    shinyjs::hide()
  }else{ # input <- list(traitPredictionsCorrelation="YLDTONHA",boxplotMtaPredsTypeCorr="environment_designation")
    
    inputPanel(
      selectInput(ns("corrplotMtaPredsType"),
                  label = "Entry type",
                  choices = entryTypes, multiple = TRUE,
      ),
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    plotly::renderPlotly({
      ...
      if(length(input$corrplotMtaPredsType) > 0){
        ...
        wide2 <- do.call(cbind,level3[which(N == N[1])])
        corr <- round(stats::cor(wide2, use="pairwise.complete.obs"),2)
        mydata4 <- cgiarBase::matToTab(corr)
        p <- ggplot2::ggplot(data = mydata4, ggplot2::aes(Var2, Var1, fill = Freq))+
          ggplot2::geom_tile(color = "white")+
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    shinyjs::hide()
  }else{ # input <- list(biplotMtaPredsType=entryTypes[1:4],traitBiplot=traits[1])
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iType3 in entryTypes){
      ...
      for(iTrait3 in traits){
        ...
        level3 <- lapply(rapply(level2, enquote, how="unlist"), eval)
        N <- sort(unlist(lapply(level3,length)), decreasing = TRUE)
        pred.mta.biplot <- do.call(cbind,level3[which(N == N[1])])
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    shinyjs::hide()
  }else{ # input <- list(scatterMtaPredsType=entryTypes[1], scatterMtaPredsTrait=traits[1], scatterMtaPredsEnvir=envirTypes[1])
    inputPanel(
...
if (is.null(result)){
  ...
  pred.mta <- subset(pred, select = c(trait,designation,environment,entryType,predictedValue))
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    shinyjs::hide()
  }else{ # input <- list(corrplotTraitMtaPredsType=entryTypes[1],corrplotTraitMtaPredsEnv=envirTypes[1])
    
    inputPanel(
      selectInput(ns("corrplotTraitMtaPredsType"),
                  label = "Entry type",
                  choices = entryTypes, multiple = FALSE,
      ),
