...
if (is.null(result)){
  ...
  traits2 <- unique(result$modeling[which(result$modeling$analysisId == idSta),"trait"])
    gxeList <- list("gxeCS" = c("environment_designation", "designation_environment", 
                              "environment:designation", "designation:environment"),
                  "gxeDMD" = c(paste0("env",unique(envirTypes),"_designation"), paste0("designation_env",unique(envirTypes)),
                               paste0("env",unique(envirTypes),":designation"), paste0("designation:env",unique(envirTypes))),
  ...
  if(gxeModelNum == 2){
    effectTypes2 <- setdiff(effectTypes, c(effectTypes[which(effectTypes %in% gxeList[[2]])], 
                                             effectTypes[which(effectTypes %in% paste0("env",unique(modelingFieldsIncludedInMta$environment)))]))
