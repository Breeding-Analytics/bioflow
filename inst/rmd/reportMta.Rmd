...
`r if( sum(unlist(lapply(as.list(possibleIndices), function(x){length(grep(x,pred$trait))})))>0 ) {"### Trait by trait-sensitivity plots\n
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    if( sum(unlist(lapply(as.list(possibleIndices), function(x){length(grep(x,pred$trait))})))>0  ){ # if there is envIndex fitted
...
if (is.null(result)){
  ...
  if( sum(unlist(lapply(as.list(possibleIndices), function(x){length(grep(x,pred$trait))}))) > 0  ){ # if there is weather info
    ...
    if("params" %in% ls()){ # we will download the document
      ...
      for(iTrait in traits){ # iTrait="Yield_Mg_ha"
        ...
        for(iCovariate in covariates){ 
          ...
          subPred <- pred[which(pred$trait %in% c(input$traitMta3,input$envCovMta3) ),]
