  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iTrait in traits){
      ...
      p <- temp.pred.box %>%
        ...
        ggplot2::geom_text(data=temp.pred.box,aes(x= envMean, y = envMean , label =round(value,2) ), position=position_dodge(width = 0.8),size = 3, vjust = -0.5,colour="black") 
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iTrait in traits){
      ...
      # nagm2 <- nagm[goodFields,goodFields]
...
if (is.null(result)){
  ...
  if("params" %in% ls()){  # if user will download the report
    ...
    shiny::selectInput(ns("parameterMetrics2"), 
                       label = "Parameter:", 
                       multiple = FALSE,
                       selected = setdiff(unique(parameters),"CV"),
                       choices = setdiff(unique(parameters),"CV")
    )
  }
}
```
