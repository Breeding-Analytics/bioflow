...
`r if( any( unlist(lapply( as.list(unique(result$metadata$weather$trait)), function(x){
...
if (is.null(result)){
  ...
  if(length(checksFound) > 0){
    ...
    predList <- split(pred, pred$trait) # make a list by trait
    
    for(iTrait in 1:length(predList)){
      x <- predList[[iTrait]]
      byCheck <- list()
      ...
      wide <- reshape(long[, c("benchmark", "designation","predictedValue2")], direction = "wide", idvar = "designation",
                      timevar = c("benchmark"), v.names = c("predictedValue2"), sep= "_")
      colnames(wide) <- paste0(names(predList)[iTrait], gsub("predictedValue2","",colnames(wide)))
      colnames(wide)[1] <- "designation"
      if(iTrait == 1){
        res <- wide
      }else{
        res <- merge(res, wide, by="designation")
      }
    }
    numCols <- colnames(res)[-c(1)]
