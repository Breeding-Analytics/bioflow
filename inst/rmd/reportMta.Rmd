...
if (is.null(result)){
  ...
  if(length(result$metrics[result$metrics$analysisId == idMta & result$metrics$parameter == "nEnv" & result$metrics$value > 1, "trait"]) > 0){
    ...
    for(itt in traitsBiplotAll){
      idSta <- result$modeling[result$modeling$analysisId == idMta & result$modeling$trait == "inputObject","value"]
      pred.staDT <- result$predictions
      pred.staDT <- pred.staDT[pred.staDT$analysisId == idSta,]
      
      goodFields <- modelingFieldsIncludedInMta[modelingFieldsIncludedInMta$value == "TRUE" & modelingFieldsIncludedInMta$trait == itt, "environment"]
      pred.mta.biplotDT <- reshape(pred.staDT[which(pred.staDT$trait == itt & pred.staDT$environment %in% goodFields ),c("designation","environment","predictedValue")], 
                                   direction = "wide", idvar = "designation", timevar = "environment")
      
      rownames(pred.mta.biplotDT) <- pred.mta.biplotDT[,"designation"]
      pred.mta.biplotDT <- pred.mta.biplotDT[,-1, drop=FALSE]
      names(pred.mta.biplotDT) <- gsub("predictedValue.","",names(pred.mta.biplotDT))
      
      # keep only
      missing <- apply(pred.mta.biplotDT,1, function(x){length(which(is.na(x)))/length(x)})
      pred.mta.biplotDT <- pred.mta.biplotDT[which(missing < 0.8),, drop=FALSE]
      missing <- apply(pred.mta.biplotDT,2, function(x){length(which(is.na(x)))/length(x)})
      pred.mta.biplotDT <- pred.mta.biplotDT[,which(missing < 0.8), drop=FALSE]
      pred.mta.biplotDT <- apply(pred.mta.biplotDT,2, sommer::imputev)
      
      prin_compDT <- prcomp(pred.mta.biplotDT, rank = 2)
      componentsDT <- prin_compDT[["x"]]
      componentsDT <- data.frame(componentsDT)
      componentsDT$PC2 <- -componentsDT$PC2
      componentsDT$trait <- itt
      bpl[[itt]] <- componentsDT
    }
    
    componentsDTFinal <- do.call(rbind, bpl)
    
    
    if("params" %in% ls()){ # we will download the document
      printTable(componentsDTFinal)
    }else{
      DT::renderDT({DT::datatable(
        componentsDTFinal[which(componentsDTFinal$trait == input$traitBiplot),],
        rownames = FALSE,
