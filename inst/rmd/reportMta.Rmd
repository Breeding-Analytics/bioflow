...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iTrait in traits){
      ...
      p <- ggplot2::ggplot(
        ...
        ggplot2::geom_errorbar(ggplot2::aes(ymax = predictedValue + stdError,
                               ...
                               width = 0.10) +
        ggplot2::labs(title = "Across environment estimates comparison", x = "Designation", y = "Predicted value", col = "entry type") +
        ggplot2::theme(axis.text.x = element_text(angle = 45, hjust=1, vjust = 1 , size = 7), legend.position = "bottom") +
        guides(fill=guide_legend(nrow=3,byrow=TRUE))
      if(length(unique(temp.pred.box$designation)) > 1200){ # go fixed format
        print(p)
      }else{ # go dynamic
        print(htmltools::tagList(plotly::ggplotly(p)))
      }
    ...
    if(length(unique(pred$designation)) > 1200){ # go fixed format
      ...
      shiny::renderPlot({
        ...
        p <- ggplot2::ggplot(
          ...
          ggplot2::geom_errorbar(ggplot2::aes(ymax = predictedValue + stdError,
                                 ...
                                 width = 0.10) +
          ggplot2::labs(title = "Across environment estimates comparison", x = "Designation", y = "Predicted value", col = "entry type") +
          ggplot2::theme(axis.text.x = element_text(angle = 45, vjust = 0.5, size = 7, face = "bold"), legend.position = "bottom") + ggplot2::guides(fill=guide_legend(nrow=3,byrow=TRUE))
        p
