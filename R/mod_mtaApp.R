    ...
    mainPanel(width = 12,
              tabsetPanel( id=ns("tabsMain"),
                           ...
                           tabPanel(div(icon("book"), "Information") ,
                                    br(),
                                    column(width = 6,
                                           h1(strong(span("Multi Trial Analysis Module Using LMMsolver", tags$a(href="https://www.youtube.com/watch?v=rR1DhTt25n4&list=PLZ0lafzH_UmclOPifjCntlMzysEB2_2wX&index=7", icon("youtube") , target="_blank"), style="color:darkcyan"))),
                           ...
                           tabPanel(div(icon("arrow-right-to-bracket"), "Input steps"),
                                    tabsetPanel(
                                      ...
                                      tabPanel(div( icon("dice-two"), "Select traits", icon("arrow-right") ), #icon = icon("dice-two"),
                                               ...
                                               column(width=12, style = "background-color:grey; color: #FFFFFF",
                                                      ...
                                                      column(width = 8, style = "background-color:grey; color: #FFFFFF",
                                                             br(),
                                                             shinydashboard::box(width = 12, style = "color: #000000",status = "success",solidHeader=FALSE,collapsible = TRUE, collapsed = TRUE, title = "Alternative response distributions to fit...",
                                      ...
                                      tabPanel(div( icon("dice-three"), "Pick the model", icon("arrow-right") ), # icon = icon("dice-three"),
                                               br(),
                                               column(width=12, style = "background-color:grey; color: #FFFFFF",
...
mod_mtaApp_server <- function(id, data){
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(c(data(),input$version2Mta, input$trait2Mta), {
      ...
      req(input$trait2Mta)
      dtMta <- data()
      metaPheno <- dtMta$metadata$pheno[which(dtMta$metadata$pheno$parameter %in% c("environment","year","season","timepoint","country","location","trial","study","management")),]
      metaPheno <- metaPheno[which(!duplicated(metaPheno$value)),] # avoid error when user maps columns twice
      otherMetaCols <- unique(dtMta$data$pheno[,metaPheno$value,drop=FALSE])
      colnames(otherMetaCols) <- cgiarBase::replaceValues(Source = colnames(otherMetaCols), Search = metaPheno$value, Replace = metaPheno$parameter )
      otherMetaCols <- otherMetaCols[which(!duplicated(otherMetaCols[,"environment"])),,drop=FALSE] # we do this in case the users didn't define the environment properly
