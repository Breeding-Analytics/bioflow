...
mod_indexDesireApp_server <- function(id, data){
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(c(data()), {
      ...
      dtIdxD <- data()
      dtIdxD <- dtIdxD$status
      dtIdxD <- dtIdxD[which(dtIdxD$module %in% c("mta","mtaFlex","mtaLmms","mas", "mtaAsr") ),]
      traitsIdxD <- unique(dtIdxD$analysisId)
    ...
    outIdxD <- eventReactive(input$runIdxD, {
      ...
      if(sum(dtIdxD$status$module %in% c("mta","mtaFlex","mtaLmms","mas", "mtaAsr") ) == 0) {
        output$qaQcIdxDInfo <- renderUI({
          if (hideAll$clearAll)
            return()
          else
        ...
        output$qaQcIdxDInfo <- renderUI({return(NULL)})
        result <- try(cgiarPipeline::indexDesire(
        #source("C:\\Users\\RAPACHECO\\OneDrive - CIMMYT\\Documents\\GitHub\\bioflow\\indexDesire.R")
        #result <- try(indexDesire(
