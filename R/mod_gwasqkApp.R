    ...
    mainPanel(width = 12,
              tabsetPanel( id=ns("tabsMain"),
                           ...
                           tabPanel(div(icon("arrow-right-to-bracket"), "Input steps"),
                                    tabsetPanel(
                                      tabPanel("Pick STA or MTA-stamp", icon = icon("dice-one"),
                                               br(),
                                               column(width=12, style = "background-color:grey; color: #FFFFFF",
                                                      column(width=3, selectInput(ns("version2Gwas"), "STA/MTA version to analyze (required)", choices = NULL, multiple = TRUE)),
                                                      column(width=3, selectInput(ns("versionMarker2Gwas"), "Marker QA version to use (required)", choices = NULL, multiple = TRUE)),
                                                      column(width=3, selectInput(ns("env2Gwas"), "Environment to use (required)", choices = NULL, multiple = TRUE)),
  ...
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(
      ...
      if(!is.null(data())) {
        ...
        dtGwas <- dtGwas[which(dtGwas$module %in% c("mta")),]
    ...
    observeEvent(c(data()), {
      ...
      dtGwas <- data()
      dtGwas <- dtGwas$status
      dtGwas <- dtGwas[which(dtGwas$module %in% c("sta","mta")),]
    ...
    output$barplotPredictionsMetrics <- plotly::renderPlotly({ # update plot
      ...
      idGwas <- data()$status[which(data()$status$module %in% "mta"),"analysisId"]
    ...
    output$phenoGwas <-  DT::renderDT({
      ...
      envGwasSel <- data()
      envGwasSel <- envGwasSel$status
      envGwasSel <- envGwasSel[which(envGwasSel$module %in% c("mta")),]
      envGwasSel <- envGwasSel[length(envGwasSel)]
      if(input$version2Gwas %in% envGwasSel){
        dtGwas <- dtGwas
    ...
    outGwas <- eventReactive(input$runGwas, {
      ...
      if(sum(dtGwas$status$module %in% c("mta")) == 0) {
