...
mod_PopStrApp_server <- function(id, data){
  moduleServer( id, function(input, output, session){
    ...
    outPopStr <- eventReactive(input$runPopStr, {
        ...
        updateTextInput(session,'tx','X Axis Label',value = paste0('Factor 1 (',uno[[3]][1],'%)'))
        updateTextInput(session,'ty','Y Axis Label',value = paste0('Factor 2 (',uno[[3]][2],'%)'))
        updateTextInput(session,'tz','Z Axis Label',value = paste0('Factor 3 (',uno[[3]][3],'%)'))
        
        #Actualiza el selectinput de acuerdo a la base de datos cargada
        updateSelectInput(session,'xcol', 'X Variable',choices = names(uno[[5]]),selected=names(uno[[5]])[2])
        updateSelectInput(session,'ycol', 'Y Variable',choices = names(uno[[5]]),selected=names(uno[[5]])[3])
        updateSelectInput(session,'zcol', 'Z Variable',choices = names(uno[[5]]),selected=names(uno[[5]])[4])
        updateSelectInput(session,'catv', 'Group',choices = names(uno[[5]]),selected=names(uno[[5]])[5])
        updateSelectInput(session,'catvdend', 'Group',choices = names(uno[[5]]),selected=names(uno[[5]])[5])
        updateSelectInput(session,'eti', 'Label',choices = names(uno[[5]]),selected=names(uno[[5]])[1])
        
        return(list(result=result,uno=uno))
        })
      ...
      #Ver datos en tabla dinamica Summary Diversity
        output$seeDataDiver<-DT::renderDT({
          allInfo=DivResult()[["result"]][["PopStr"]]
        ...
        output$seeDataGDiver<-DT::renderDT({
          datos<-as.data.frame(DivResult()[["uno"]][[1]])
          datos1<-as.data.frame(DivResult()[["uno"]][[5]])
          seedatosFst=cgiarBase::gdiv(datos,datos1,as.character(input$catv),as.character(input$quitomono),as.data.frame(DivResult()[["uno"]][[2]]))
        ...
        output$seeDataStatMark<-DT::renderDT({
          seedatosStaM<-as.data.frame(DivResult()[["result"]][["PopStr"]][[4]])
        ...
        output$seeDataStatGeno<-DT::renderDT({
          seedatosStaG<-as.data.frame(DivResult()[["result"]][["PopStr"]][[5]])
        ...
        output$seeDataMDS<-DT::renderDT({
          seedatosMDS<-as.data.frame(DivResult()[["uno"]][[5]])
        ...
        output$try=plotly::renderPlotly({
          ...
          if(!is.null(mydata)){
            p=plotly::plot_ly(data=DivResult()[["uno"]][[5]],x=DivResult()[["uno"]][[5]][,input$xcol],y=DivResult()[["uno"]][[5]][,input$ycol],color=DivResult()[["uno"]][[5]][,input$catv],
                              type="scatter",mode="markers",colors = input$color,xaxis=F, yaxis=F,
                              text=DivResult()[["uno"]][[5]][,input$eti],marker=list(size=input$size))
        ...
        output$heat=plotly::renderPlotly({
          distMat=DivResult()[["result"]][["PopStr"]][["Distance_Matrix"]]
          if(!is.null(distMat)){
        ...
        observeEvent(input$catv,{
          ...
          var=as.factor(DivResult()[["uno"]][[5]][,input$catv])
          grupos=nlevels(var)
        ...
        observeEvent(input$catvdend,{
          ...
          colores=colors()[-c(1,3:12,13:25,24,37:46,57:67,80,82,83,85:89,101:106,108:113,126:127,138,140:141,152:253,260:366,377:392,
                              394:447,449,478:489,492,513:534,536:546,557:561,579:583,589:609,620:629,418,436,646:651)]
          d=sample(colores,100)
          var=as.factor(DivResult()[["uno"]][[5]][,input$catvdend])
          grupos=nlevels(var)
