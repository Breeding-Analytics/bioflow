...
if (is.null(resultMta)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    cat(paste0("### Distribution  {.tabset .tabset-pills}    \n\n  "))
...
if (is.null(resultMta)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iTrait in traits){
      ...
      # keep only as many columns as available or up to the 4th PC
      pred.mta.biplot <- na.omit(pred.mta.biplot[,2:min(c(ncol(pred.mta.biplot),4))])
