...
suppressWarnings(tryCatch({
  if(file.exists("./outputs/resultMta.RData")){
...
if (is.null(resultMta)){
  ...
  # resultMta$status <- resultMta$status[1:6,]
  ...
  traits <- unique(resultMta$modeling[which(resultMta$modeling$analysisId == idMta  & resultMta$modeling$environment == "across"),"trait"]) # unique(pred$trait)
}
```
...
if (is.null(resultMta)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iTrait in traits){
      ...
      p.cor.pred.env <- corPlotPredictions(predictions.gcorrE, iTrait, unitOfCorrelation="designation", correlatedAcross="environment",
...
if (is.null(resultMta)){
  ...
  pred.mta <- resultMta$predictions
  pred.mta <- pred.mta[pred.mta$analysisId == idMta,]
  traits <- unique(resultMta$modeling[which(resultMta$modeling$analysisId == idMta  & resultMta$modeling$environment == "across"),"trait"]) # unique(pred.mta$trait)
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    for(iTrait in traits){
      ...
      pred.mta.biplot <- subset(pred.sta, select = c(trait,designation,environment,predictedValue)) %>%
        filter(trait== iTrait) %>% droplevels() %>%
        pivot_wider(names_from = environment, values_from = predictedValue) %>%
        select(-trait) %>% column_to_rownames("designation")
